...
const Login = () => {
    const {login, user, pass, setRole, logout} = useAuthContext();
    ...
    const handleFormSubmit = async (user, userId, password) => {
        login(userId)
        pass(password)
        const url=`https://agamitechinterviewapi.onrender.com/api/v1/handleFormSubmit`
  ...
  return (
    <div>
     <form onSubmit={(e) => e.preventDefault()}>
        ...
        <button type="submit" onClick={() => handleFormSubmit(user, userId, password)}>Submit</button>
      </form>

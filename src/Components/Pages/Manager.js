    ...
    const fetchEmployees = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/v1/manager/employees/${managerId}`
        );
    ...
    const fetchTimesheets = async () => {
      try {
        ...
        const response = await axios.post(
          'http://localhost:5000/api/v1/manager/all-timesheets',
          { employeeIds }
        );
  ...
  const handleGradeUpdate = async (timesheetId) => {
    try {
      const response = await axios.put(
        `http://localhost:5000/api/v1/manager/grade/${timesheetId}`,
        {
      ...
      setTimesheets((prevTimesheets) =>
        prevTimesheets.map((timesheet) =>
          timesheet._id === timesheetId
            ? { ...timesheet, managerGrade: selectedGrade }
            : timesheet
        )
      );
          ...
          {timesheets.map((timesheet) => (
            <li key={timesheet._id}>
              ...
              <li>
                ...
                <button onClick={() => handleGradeUpdate(timesheet._id)}>
                  Grade
                </button>
              </li>
              <br />

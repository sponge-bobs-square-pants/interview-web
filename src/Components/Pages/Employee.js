...
const Employee = () => {
  ...
  const submitTimesheet = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:5000/api/v1/submit-timesheet',
        data
      );
      ...
      if (!latestTimesheet || latestTimesheet.managerGrade === null) {
        ...
        console.log(
          'You cannot edit a timesheet that has been graded by the manager.'
        );
      }
    } catch (error) {
  ...
  const getTimesheets = async (user) => {
    try {
      const url = `http://localhost:5000/api/v1/getetimesheet?employee=${user}`;
  ...
  const getManager = async (user) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/v1/getmyManager?employee=${user}`
      );
  ...
  const handleUpdateTimesheet = async (editTimesheetId) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/api/v1/update-timesheet/${editTimesheetId}`,
        data
      );
      alert(response.data);
  ...
  const RandomNumber = async () => {
    ...
    const response = await axios.get(
      'http://localhost:5000/api/v1/LuckyNumber',
      config
    );
  ...
  return (
    <div>
      ...
      <form>
        ...
        {editTimesheetId ? (
          <>
            <button onClick={() => handleUpdateTimesheet(editTimesheetId)}>
              Update
            </button>
      ...
      <button onClick={RandomNumber}>JWT TESTING</button>
    </div>
  );
};
